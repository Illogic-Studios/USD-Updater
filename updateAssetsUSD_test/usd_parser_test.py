...
class USDParserTest(unittest.TestCase):
    def test_parse(self):
        layer_path = (
            Path(ENVIRONNEMENT_CONTEXT) / Path("parse/single_ref.usda")
        ).as_posix()
    ...
    def test_recursive_parse(self):
        layer_path = (
            Path(ENVIRONNEMENT_CONTEXT) / Path("recursive_refs/recursive_refs.usda")
        ).as_posix()
        layer = Sdf.Layer.FindOrOpen(layer_path)
    ...
    def test_update(self):
        layer_path = (
            Path(ENVIRONNEMENT_CONTEXT) / Path("update/single_ref.usda")
        ).as_posix()
        ...
        usdp.update_layer(layer, mode=usd_parser.USDParser.UpdateMode.OVERWRITE)
        usdp.parse(layer)
        item_list: list[AssetItem] = usdp.get_assets_to_update()
    ...
    def test_update_multiples_refs(self):
        ...
        layer_path = (
            Path(ENVIRONNEMENT_CONTEXT) / Path("multiples_refs/multiple_refs.usda")
        ).as_posix()
        layer_root, layer_ext = os.path.splitext(layer_path)
        layer_copy_path = layer_root + "_test_update" + layer_ext
    ...
    def test_resolve_path(self):
        ...
        abs_path = (
            Path(ENVIRONNEMENT_CONTEXT) / Path("parse/v008\common.usda")
        ).as_posix()
        dir_path = (Path(ENVIRONNEMENT_CONTEXT) / Path("parse")).as_posix()
        ...
        self.assertEqual(
            expected_failed_path, usdp._resolve_path(failed_path, dir_path)
        )
