...
if _ENV_QT_FROM_PRISM:  # pragma: no cover
    if not import_qtpy():
        logger.error("qtpy not found in C:/ILLOGIC_APP/Prism")
        sys.exit(1)
...
class MainInterface(Qt.QMainWindow):
    ...
    def __init__(
        ...
        if self.openType == "maya":  # pragma: no cover
            targetModeItem = ["Update from USD file", "Update Current Work Layer"]
        else:
            targetModeItem = ["Update from USD file"]
        ...
        self.targetMode.currentIndexChanged.connect(self.find_USD_file_to_update)
        ...
        self.layerList.itemSelectionChanged.connect(self.onSelectedLayerChanged)
        ...
        self.parseButton.clicked.connect(self.reload_dependencies)
    ...
    def onSelectedLayerChanged(self):
        ...
        if selectedItem:
            ...
            self.QTabLayers.setCurrentIndex(current_index.row())
    ...
    def setUpdatedLayer(self, layer: Sdf.Layer, on: bool):
        ...
        widget: LayerWidget = self.layerList.itemWidget(asset_list.layer_item)
    ...
    def browse_file(self):
        ...
        path, _ = Qt.QFileDialog.getOpenFileName(
            self, "Select USD File", choosePlace, "USD Files (*.usd *.usda *.usdc)"
        )
    ...
    def default_parse(self, pathfile: str):
        try:
            ...
            self._hiddenDependencies = self.getHiddenDependencies(self._pathFiles)
            ...
            self.log(f"Error loading USD file: {e}", severity=logging.WARNING)
    ...
    def ask_latest(self, pathfile: str):
        ...
        if last_version != current_version:
            ...
            ask_latest.setDefaultButton(Qt.QMessageBox.StandardButton.Cancel)
        ...
        for node in nodes:
            if node.type().name() == "prism::LOP_Import::1.0":
                try:
                    ...
                    logger.warning(
                        f"Could not find file path in parm of node {node.name()}"
                    )
            else:
        ...
        if self.openType == "maya":  # pragma: no cover
            ...
            if not scene_path:
                if os.path.exists(str(filepath)):
                    ...
                    logger.debug("Did not found path from node fallback to scenepath")
                    return []
            ...
            self.log(
                "Error loading USD file : pas de file scene donné",
                severity=logging.WARNING,
            )
            return []
    ...
    def getHiddenDependencies(self, exports_path):
        ...
        for path in exports_path:
            ...
            if parent is not None:
                if parent not in exports_path and parent not in hidden_depencencies:
                    hidden_depencencies.append(parent)
                grand_parent = self.getParentLayer(parent)
                if (
                    grand_parent is not None
                    and grand_parent not in exports_path
                    and grand_parent not in hidden_depencencies
                ):
    ...
    def load_maya_work_layer(self):  # pragma: no cover
        ...
        if not stage:
            self.log("No valid USD stage selected in Maya.", severity=logging.ERROR)
            return
    ...
    def get_selected_stageMaya(self) -> Usd.Stage:  # pragma: no cover
        ...
        if maya_stage_node:
            ...
            if node_type != "mayaUsdProxyShape":
                logger.debug(
                    "Selected node is not of type mayaUsdProxyShape."
    ...
    def add_item_list(self, listwidget: AssetListWidget, item: AssetItem):
        ...
        if item.can_be_updated:
            ...
            checkbox.stateChanged.connect(
                lambda state, i=item: setattr(
                    i, "should_be_updated", state == Qtc.Qt.Checked
                )
            )
        else:
    ...
    def build_item_list(self, assetsToUpdate: list[AssetItem]):
        ...
        for i, item in enumerate(assetsToUpdate):
            ...
            if i == 0:
                ...
                self.add_item_text(
                    asset_list, f"Current layer : <b>{previous_layer}</b>"
                )
    ...
    def add_layer_tab(
        self, layer: Sdf.Layer, asset_list: AssetListWidget, hidden: bool = False
    ):
        ...
        if len(layer_path.parts) < 3:
            ...
            layer_text = f"<b>{layer_path.parts[-3]}</b><br> - {layer_path.parts[-1]}"
    ...
    def update_all(self):
        ...
        tabs = [
            self.QTabLayers.widget(tab_idx)
            for tab_idx in range(self.QTabLayers.count())
        ]
        for tab in tabs:
            self.run_update(tab, clear_log=False, check_nodes=False)
    ...
    def run_update(self, tab, clear_log=True, check_nodes=True, clean_tabs=True):
        ...
        if not tab.can_be_updated:
            self.log(
                "Cannot update recursive found dependencies", severity=logging.WARNING
            )
            return
        ...
        new_layer = self._usd_parser.update_layer(
            current_layer, self.update_mode, self.pcore
        )
        ...
        if self.update_mode == usd_parser.USDParser.UpdateMode.NEW_VERSION:
            if new_layer is None:
                self.log(
                    f"Could not increment version for : {layer_identifier}",
                    severity=logging.ERROR,
                )
                return
            else:
...
def startUpdateAssetsUSD(openType, tmpfile=None, prism_core=None, ar_context=None):
    instance = None
